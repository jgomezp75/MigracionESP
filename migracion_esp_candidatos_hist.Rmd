...
for(x in 1:length(fileNames)) {
        ...
        if (fileNames[x] == "temp/01-Candidatos_Datos_Personales.csv") {
                # Convertir teléfonos a formato numérico
        ...
        if (fileNames[x] == "temp/02-Candidatos_Formacion_Parte1.csv") {
                # Si el título está vacío concatenar NIVEL y AREA
        ...
        if (fileNames[x] == "temp/17-Candidatos_Otros_Datos_Documentos.csv") {
                # Dejar únicamente el nombre del archivo en lugar de toda la URL
                #antes, sacar las URLs a un archivo para descargarlas después
                t %>% select(DOCUMENTO) %>% fwrite("output/adjuntos/descargar_cvs.csv")
                t$DOCUMENTO <- as.character(t$DOCUMENTO)
                t$DOCUMENTO <- substr(t$DOCUMENTO, nchar("http://cdn.epreselec.com/bbva/Documentos/")+1, nchar(t$DOCUMENTO))
                t$DOCUMENTO <- as.factor(t$DOCUMENTO)
        }
        if (fileNames[x] == "temp/55-Candidatos_Documentos_Back") {
                # Dejar únicamente el nombre del archivo en lugar de toda la URL
                #antes, sacar las URLs a un archivo para descargarlas después
                t %>% select(DOCUMENTO) %>% fwrite("output/adjuntos/descargar_informes.csv")
                t$DOCUMENTO <- as.character(t$DOCUMENTO)
                t$DOCUMENTO <- substr(t$DOCUMENTO, nchar("http://cdn.epreselec.com/bbva/Documentos/")+1, nchar(t$DOCUMENTO))
                t$DOCUMENTO <- as.factor(t$DOCUMENTO)
        }
        55-Candidatos_Documentos_Back
        fwrite(t,fileNamesOut[x], sep = "~")
        rm(t)
        gc()
}
